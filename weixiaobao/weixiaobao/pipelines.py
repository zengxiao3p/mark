...
class WeixiaobaoPipeline(object):
    def __init__(self):
    ...
    def _conditional_insert(self, tx, item):
        try:
            ...
            tx.execute(querysql2, (item['theme_name'], int(2), item['source_name']))
            result2 = tx.fetchone()
            # insert
            if result == None:
                ...
                smartpush_scrapy_id = [int(result2['smartpush_scrapy_id'])][0];
                smartpush_scrapy_detail_id=int(result['id'])
                dt = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                item["publish_date"] = item["publish_date"].strip('[]')
                sql = "update  smartpush_scrapy_detail set title=%s,publish_date=%s,scrapy_link=%s,goto_link=%s,scrapy_id=%s,created_time=%s where id = %s"
                paramsInsert = (
                    item["title"], item["publish_date"], item['scrapy_link'], item['goto_link'], smartpush_scrapy_id,
                    dt,smartpush_scrapy_detail_id)
                tx.execute(sql, paramsInsert)
