...
scrapy_link = 'http://www.boc.cn/bcservice/bi3/bi32/index.html'
...
class ToScrapeCSSSpider(scrapy.Spider):
    ...
    name = "zgyh_original_bank_dqyh"
    allowed_domains = [domain]
    start_urls = [
        scrapy_link,
    ]
    custom_settings = {
        'ITEM_PIPELINES': {'zhongguoyinhang.pipelines.ZhongguoyinhangDqyhPipeline': 302}
    }
        ...
        try:
            ...
            for showcss in response.css('div.main>div.news>ul.list>li'):
                ...
                if (showcss.css('a::attr("href")').extract_first().startswith('http') == False):
                    item['goto_link'] = "http://www.boc.cn/bcservice/bi3/bi32/" +showcss.css('a::attr("href")').extract_first()
                else:
                    item['goto_link'] = showcss.css('a::attr("href")').extract_first()
                if currentpage==1:
                    item['scrapy_link'] = "http://www.boc.cn/bcservice/bi3/bi32/index.html"
                else:
                    item['scrapy_link'] = "http://www.boc.cn/bcservice/bi3/bi32//index_"+str(currentpage-1)+".html"
                yield item

            if currentpage < pagesize:
                nextpage = int(currentpage) + 1 -1
                next_page_url = "http://www.boc.cn/bcservice/bi3/bi32/index_"+str(nextpage)+".html"
