...
class GongshangyinhangQgyhPipeline(object):
    # pipeline默认调用
    ...
    def _conditional_insert(self, tx, item):
        try:
            # 首先去重 #scrapyid = 20
            querysql = "select * from smartpush_scrapy_detail where title = %s and scrapy_id='20'"
            ...
            if result == None:
                ...
                tx.execute(querysql2, ('工商银行',int(1), '工商银行全国活动'))
                result2 = tx.fetchone()
                if  result2  != None:
