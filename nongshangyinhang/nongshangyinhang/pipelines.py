...
class NsyhOriginalBankYhxxPipeline(object):
    # pipeline默认调用
    ...
    def _conditional_insert(self, tx, item):
        try:
            # 首先去重
            querysql = "select * from smartpush_scrapy_detail where title = %s  AND title= %s"
            ...
            if result == None:
                ...
                tx.execute(querysql2, ('农商银行',int(1), '农商银行优惠信息'))
                result2 = tx.fetchone()
                if  result2  != None:
                    smartpush_scrapy_id = [int(result2['smartpush_scrapy_id'])][0]
                    dt = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                    item["publish_date"] = item["publish_date"].strip('[]')
                    sql = "insert into smartpush_scrapy_detail(title,scrapy_link,publish_date,goto_link,scrapy_id,created_time) values(%s,%s,%s,%s,%s,%s)"
                    paramsInsert = (
                        item["title"], item['scrapy_link'],item['publish_date'], item['goto_link'], smartpush_scrapy_id,
                        dt)
...
class NsyhOriginalBankXptjPipeline(object):
    # pipeline默认调用
    def process_item(self, item, spider):
        query = dbpool.runInteraction(self._conditional_insert, item)  # 调用插入的方法
        query.addErrback(self._handle_error, item, spider)  # 调用异常处理方法
        ...
        try:
            ...
            if result == None:
                ...
                querysql2 = "SELECT smartpush_scrapy_id FROM(SELECT a.id AS themeid , b.fromsource_id ,b.id AS smartpush_scrapy_id FROM smartpush_scrapy_theme AS a  RIGHT JOIN smartpush_scrapy AS b ON  a.id = b.theme_id" \
                            " AND a.name= %s )AS vv LEFT JOIN smartpush_from_source AS sfs ON  vv.fromsource_id=sfs.id  " \
                            "WHERE   sfs.type =%r AND sfs.name= %s "
                tx.execute(querysql2, ('农商银行',int(1), '农商银行新品推介'))
                result2 = tx.fetchone()
                if  result2  != None:
                    smartpush_scrapy_id = [int(result2['smartpush_scrapy_id'])][0]
                    dt = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")
